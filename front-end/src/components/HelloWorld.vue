<template>
  <div class="X" v-for="tarefa in tarefas.projects" :key="tarefa.id">
    <ul class="SG">
      <li class="sgLi">
        <div class="box">
        <h3>{{ tarefa.name }}</h3>
        <ul class="df">
            <li>ID: {{ tarefa.id }}</li>
            <li>Nome: {{ tarefa.name }}</li>
            <li>Identificação: {{ tarefa.identifier }}</li>
            <li>Descrição: {{ tarefa.description == '' ? "Sem descrição":  tarefa.description  }}</li>
            <li>Status: {{ tarefa.status }}</li>
            <li>Projeto publico: {{ tarefa.is_public == true ? "Sim": "Não" }}</li>
            <li>Data criação: {{ tarefa.created_on.substr(0, 10) }}</li>
            <li>Data de update: {{ tarefa.updated_on.substr(0, 10) }}</li>
        </ul> 
      </div>
    </li> 
  </ul>
...
export default {
  methods: {
    ...mapActions(["listarTodosProjetos"])
  },
  computed: {
    ...mapState(["tarefas"])
  },
  created() {
    this.listarTodosProjetos()
  }
}
</script>
<style scoped>
